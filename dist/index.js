...
const getItemFromKeys = (arr, pairs) =>
  toPairs(pairs).reduce((acc, pair$$1) =>
    filter(propEq(pair$$1[0], pair$$1[1]))(acc)
  , arr)[0];
...
const replace$1 = (arr, item, key, value) =>
  item
...
const remove$1 = (arr, key, value) =>
  key
    ? removeItem(arr)(key, value)
    : removeItem(arr);

// remove properties from an item
const removeProps = (arr, props$$1, key, val) =>
  props$$1
    ? replace$1(arr, omit(props$$1, get(arr, key, val)), key, val)
    : replace$1(arr);
...
const update$1 = (arr, item, key, value) =>
  item
...
const updateItemProperties = arr => (updates, k, v) => {
  const {key, value} = getKeyVal(k, v);
  replace$1(arr, {...get(arr, {[key]: value}), ...updates}, key, value);
  return arr;
};
...
const addItem = arr => (item, sortfield) => {
  arr.unshift(item);
  sort$1(arr, sortfield);
  return arr;
};
const add$1 = (arr, item, sortfield) =>
  item
...
exports.getKeyValItem = getKeyValItem;
